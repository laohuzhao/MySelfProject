package com.example.xitong.zookeeper.test2;
...
public class ZookeeperDistributeLock implements DistributedLock {
    ...
    private String getLockNodeNumber(String str, String lockName) {
        ...
        int index = str.lastIndexOf(lockName);//指定前缀在字符串中的索引值
        if (index >= 0) {
        ...
        /*
        if (index >= 0) {//索引值大于=0说明有节点
            index += lockName.length();
            return index <= str.length() ? str.substring(index) : "";//true截取字符串后半截,false返回“”；
        }
        return str;*/
    }
    ...
    private List<String> getSortedChildren() throws Exception {
        List<String> children = zooKeeper.getChildren(rootPath, false);//获取所有子节点
        if (children != null && !children.isEmpty()) {
            Collections.sort(children, new Comparator<String>() {
                /*compareTo相等返回0，前大于后返回1，前小于后返回-1,compare的方法根据-1 0 1的判断自动对children进行从小到大排序并返回集合*/
    ...
    private boolean waitToLock(long startMillis, Long millisToWait) throws Exception {
        ...
        try {
            while (!haveTheLock) {//循环获取锁
                logger.info("get Lock Begin");
                // 该方法实现获取locker节点下的所有顺序节点，并且从小到大排序,
                List<String> children = getSortedChildren();
                String sequenceNodeName = currentLockPath.substring(rootPath.length() + 1);//当前节点截取

                // 计算刚才客户端创建的顺序节点在locker的所有子节点中排序位置，如果是排序为0，则表示获取到了锁
                int ourIndex = children.indexOf(sequenceNodeName);//没有返回-1，有返回字符串在集合的下标
                ...
                // 此时当前客户端需要等待其它客户端释放锁，
                boolean isGetTheLock = ourIndex == 0;//布尔类型的三元表达式isGetTheLock = ourIndex == 0？true:false;
                //当这个元素下标等于0的时候说明轮到它了。
                ...
                if (isGetTheLock) {
                    ...
                    haveTheLock = true;//如果isGetTheLock为true,表示获取到锁了，赋值haveTheLock为true,退出死循环
                } else {
                    ...
                    final Watcher previousListener = new Watcher() {
                        public void process(WatchedEvent event) {
                            if (event.getType() == EventType.NodeDeleted) {//删除当前线程节点触发函数
                                latch.countDown();
                            }
                        }
                    };

                    // 如果节点不存在会出现异常，判断节点是否存在并添加监听器
